 ...
 */
abstract class SocialPostsWorkerBase extends QueueWorkerBase implements ContainerFactoryPluginInterface {
  ...
  protected function reportWork($worker, $item) {
    if ($this->state->get('social_feed_aggregator_show_status_message')) {
      drupal_set_message(
    ...
    $this->logger->get('social_feed_aggregator')->info('Queue @worker worker processed item with sequence @sequence created at @time', [
      '@worker' => $worker,
      '@sequence' => $item->sequence,
      '@time' => date_iso8601($item->created),
    ]);
